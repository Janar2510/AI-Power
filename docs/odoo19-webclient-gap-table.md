...
| Search layer | `search/search_model`, `with_search`, control panel | `search/search_model.js`, `with_search.js`, `search_bar.js`; `core/search_model.js` shim | share search across list/form/kanban | **Partial (1.248+)**: **`WithSearch(ListController)`** active when OWL **`ActionContainer`** handles list actions (**Post-1.248 P3**); form/kanban wiring still incremental |
...
| Command / hotkeys | `hotkey_service`, command palette | `services/hotkey.js`, `services/command_palette.js`; **`initHotkey`** from **`main.js`** boot | wire all shortcuts through services | **Partial (1.248+)**: **`app/main.js`** calls **`commandPalette.initHotkey()`** after **`startServices`** (Mod+K in esbuild-primary shell); full shortcut map vs Odoo not claimed |
