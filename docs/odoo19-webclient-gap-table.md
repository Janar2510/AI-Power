...
| Views | modular view packages + registries | metadata-driven; heavy logic in `main.js` / legacy | migrate dashboard/list/form/kanban into view-owned modules | **Partial (562+…+635)**: as **620** + **630–635** **`DATA_ROUTES_SLUGS` / `getModelForRoute` / `menuToRoute`** alignment, **`fleet`**, **Website/eCommerce** placeholder shells, **`__ERP_STRICT_ROUTING`** for unknown slugs; **test_main_js_route_consistency_phase631** |
