...
| Services | registry-driven service startup | `env.services` from `createModernServices`; legacy `window.Services` adapters when present | env-owned service startup with legacy adapters | **Met** (foundation): session, views, menu, theme, title, notification; **1.250.7:** `session.js` **15s** abort on `get_session_info`; modular `view_service.js` **20s** abort on `fetch` JSON-RPC; **1.250.8:** `rpc_deadline.js` **`__ERP_rpcRaceDeadline`** for OWL list/form **`orm`** waits (**25s** default); **1.250.9:** legacy **`Services.orm`** **`_callKw`** always races **`__ERP_rpcRaceDeadline`** (**25s** default) |
...
| Route apply | monolithic `routeApply` | `main.js` `routeApplyInternal` | extractable plugins | **Partial (1.250)**: **`route_apply_registry.js`** + **`installRouteApplyRegistryPlugins()`** in `main.js` — website/eCommerce placeholders remain inline; **1.250.8:** **`route_apply_plugin_keyboard_shortcuts.js`** — `#keyboard-shortcuts`; **1.250.9:** **`route_apply_plugin_client_info.js`** — `#client-info`; **1.250.10:** **`route_apply_plugin_discuss.js`** — `#discuss`; **1.250.11:** **`route_apply_plugin_reports.js`** (5 report routes) + **`route_apply_plugin_settings.js`** (apikeys, totp, dashboard-widgets, settings) extracted from `main.js` |
...
| Field widgets | `addons/web/static/src/views/fields/*`, registries | `addons/web/static/src/core/field_registry.js`, `widgets/*`, manifest order before `main.js` | align widget API with Odoo field components over time | **Partial (1.248+)**: OWL **`fieldRegistry`** + **`badge`**, **`statusbar`**, **`priority`**, **`url`**, **`email`**, **`phone`**, **`image`**, **`color_picker`** + list **`format()`** dispatch; **1.250.1:** legacy **`FieldWidgets.render`** also registers **`statusbar`** (same HTML shell as list/form wiring). **1.250.6:** CRM form **`ai_win_probability`** uses legacy **`percentage`** (**`crm_views.xml`**, **`test_field_registry`**). **1.250.8:** legacy **`boolean_toggle`** (switch markup). **1.250.9:** `is_company` XML uses `boolean_toggle`. **1.250.10:** `email`/`phone`/`monetary`/`date`/`remaining_days` wired in `base`/`sale`/`project` addon XML; `xml_loader.py` list-column parser captures `widget` attr; `remaining_days` badge + CSS tokens. Full Odoo matrix still open |
...
| Command / hotkeys | `hotkey_service`, command palette | `services/hotkey.js`, `services/command_palette.js`; **`initHotkey`** from **`main.js`** boot | wire all shortcuts through services | **Partial (1.250+)**: **`__ERP_WEBCLIENT_SHORTCUT_CONTRACT.modular`** documents **Alt+H** → `#home`, **Alt+G** → `#contacts`, **Alt+D** → `#discuss`, **Mod+K** command palette, **Alt+K** list→kanban (**`shortcutAltK`**); **`hotkey.js`** calls **`preventDefault`** for **Mod+K** / **Mod+Shift+K** so browser UI does not steal the chord. Legacy **Alt+** set in **`main.js`** (10 entries in frozen **`alt`** array); **1.250.4**: **Alt+/** → `#list-search`; **1.250.8**: **Alt+R** refresh; **1.250.9**: **Alt+G** → `#contacts`; **1.250.10**: **Alt+D** → `#discuss`. Full Odoo hotkey matrix not claimed |
