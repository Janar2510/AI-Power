...
| Phase 544 (account fiscal / multi-tax / partial — deferral) | **Shipped slices:** SO fiscal (**546**), PO + draft move line fiscal (**551**), partner default fiscal (**555**), company **`account_lock_date`** on post (**557**), move/journal **`company_id`** + per-move lock (**560**), **all-included** multi-tax chain (**563**), **`ir.sequence` per company** (**564**), **mixed include/exclude multi-tax** (**568**), **`ir.sequence` date-range + move `date`** (**569**). **Still deferred:** partial reconcile / FX implementation — design **`docs/account_partial_reconcile_design.md`** (**572**) | `docs/account_odoo19_gap_audit.md` | deferred | unittest + design doc |
