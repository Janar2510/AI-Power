...
- **Post–1.250.4 (engineering)** — **647b** / **679** / **809** unchanged; **no Phase P5 `view_service` implementation** in this train. Shipped: stability (**810** form **`read`/`default_get` deadline + Retry), **811** app-tile E2E in PR smoke + checklist, **812** list **Alt+/** → **`#list-search`** focus (contract + **`main.js`**), **813** measured-prefetch evidence note in **`odoo19_core_gap_table.md`** §808.
- **Post–1.250.6 (engineering)** — **647b** / **679** / **809** unchanged; **no P5** code. Shipped: **814a** CRM **`percentage`** widget in views; **815** generic loader skips **`ir.rule`** (mail **`model_id`** init warnings); **816** §808 log row; **817** local-dev runbook (**`DeploymentChecklist`**, **`frontend.md`**).
- **Post–1.250.7 (engineering)** — Same gates. Shipped: **session** **`getSessionInfo`** fetch **15s** abort; **ViewService** **`_jsonRpc`** **20s** abort; shell/list doc cross-references (**incremental reliability** slice from frontend next-phase plan).
- **Post–1.250.8 (engineering)** — Same gates (**647b** / **679** / **809**); no **P5 `view_service`**. Shipped: **`rpc_deadline.js`** + OWL list/form error + **Retry**; **`ActionContainer`** loading watchdog; **`#keyboard-shortcuts`** route plugin; **Alt+R** refresh route; **`boolean_toggle`** widget; **ui-ux-pro-max** mandatory rule in **`ai-rules.md`** / **`frontend.md`**; **`app-shell.md`** loading/failure spec; **`test_assets.py`** + JS tests.
- **Post–1.250.9 (engineering)** — Same gates. Shipped: **`Services.orm`** **`_callKw`** bounded by **`__ERP_rpcRaceDeadline`**; **`res.partner`** **`is_company`** **`boolean_toggle`** in XML; **Alt+G** → **`#contacts`** (legacy + modular **`hotkey`**); **`#client-info`** route plugin; **`test_views_registry`**, **`test_assets`**, shortcut contract updates.
