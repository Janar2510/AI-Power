...
class PurchaseOrderLine(Model):
    ...
    taxes_id = fields.Many2many("account.tax", string="Taxes", help="Phase 181")
    ...
    def _compute_price_subtotal(self):
        if not self:
            return []
        rows = self.read(["product_qty", "price_unit", "taxes_id"])
        env = getattr(self, "env", None)
        Tax = env.get("account.tax") if env else None
        result = []
        for r in rows:
            base = r.get("product_qty", 0) * r.get("price_unit", 0)
            tax_ids = r.get("taxes_id") or []
            if isinstance(tax_ids, (list, tuple)) and tax_ids and Tax:
                ...
                if ids:
                    ...
                    comp = taxes.compute_all(r.get("price_unit", 0), r.get("product_qty", 1))
